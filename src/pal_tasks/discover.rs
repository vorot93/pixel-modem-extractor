...
pub(crate) mod test_support {
    ...
    // -------------------------------------------------------------------
    // Complete discoverable MAIN-image builders (moved from the decompile
    // generation tests): the canonical initializer shape, the descriptor-v1
    // slot-table writer, and the three craft variants. Every consumer that
    // needs a real `pal_tasks::materialize` output — decompile generation
    // tests, decompose marshalling tests — builds its fixture from here so
    // the discoverable image exists exactly once.
    // -------------------------------------------------------------------

    pub(crate) const PAL_STRIDE: u32 = 0x1f8;
    pub(crate) const PAL_NAME_OFFSET: u32 = 0x4c;
    pub(crate) const PAL_SLOT_TABLE_OFF: u32 = 0x4000;
    pub(crate) const PAL_EXTENDED_CAPACITY: u32 = 8;

    /// Growable MAIN image under construction: bytes above `BASE` grown on
    /// demand, written at absolute addresses.
    pub(crate) struct PalImageBuilder {
        bytes: Vec<u8>,
    }
    ...
    impl PalImageBuilder {
        ...
        pub(crate) fn ensure(&mut self, end: u32) {
    ...
    impl Default for PalImageBuilder {
        fn default() -> Self {
            Self::new()
        }
    }
    ...
    pub(crate) fn pal_initializer_items(slot_base: u32, capacity: u32) -> Vec<FixtureItem> {
    ...
    pub(crate) struct PalSlotSpec {
        pub(crate) name: &'static str,
        pub(crate) entry_pointer: Option<u32>,
        pub(crate) priority: u32,
        pub(crate) stack_size: u32,
        pub(crate) opaque: u8,
    }

    pub(crate) fn pal_slot(name: &'static str) -> PalSlotSpec {
        PalSlotSpec {
            name,
            entry_pointer: None,
            priority: 0x64,
    ...
    pub(crate) fn write_pal_table(
        image: &mut PalImageBuilder,
        slot_base: u32,
        slots: &[PalSlotSpec],
    ) {
    ...
    /// runtime view).
    pub(crate) fn craft_discoverable_pal_main_image() -> Vec<u8> {
        let mut image = PalImageBuilder::new();
        let (code_bytes, _) = assemble(
            BASE,
            &pal_initializer_items(BASE + PAL_SLOT_TABLE_OFF, PAL_EXTENDED_CAPACITY),
        );
        image.write(BASE, &code_bytes);
        write_pal_table(
            &mut image,
            BASE + PAL_SLOT_TABLE_OFF,
    ...
    /// the scatter-kit fixture shape re-based to `BASE`, with copy
    /// destinations just past the image end so no scatter destination
    /// overlaps the raw span.
    pub(crate) fn craft_scatter_pal_main_image() -> Vec<u8> {
        ...
        let null_handler = BASE + 0x600;
        let copy_handler = BASE + 0x601;
        let decompress1_handler = BASE + 0x604;
        let zero_handler = BASE + 0x609;
        let sentinel_source = BASE + 0x680;
        let self_copy_source = BASE + 0x700;
        let copy_source = BASE + 0x710;
        let decompress1_source = BASE + 0x720;
        let zero_source = BASE + 0x730;
        let copy_destination = BASE + 0x5000;
        let decompress1_destination = BASE + 0x5010;
        let zero_destination = BASE + 0x5020;

        let mut image = PalImageBuilder::new();
        image.ensure(BASE + IMAGE_LEN as u32);
        // add r0, pc, #0x38; ldmia r0, {r10,r11}; add r10/r11, r0.
        image.write_u32(BASE + LOADER_OFFSET as u32, 0xe28f_0038);
        image.write_u32(BASE + LOADER_OFFSET as u32 + 4, 0xe890_0c00);
        image.write_u32(BASE + LOADER_OFFSET as u32 + 8, 0xe08a_a000);
        image.write_u32(BASE + LOADER_OFFSET as u32 + 12, 0xe08b_b000);
        ...
        image.write(BASE + 0x700, &[0xff, 0xff, 0xff, 0xff]);
        image.write(BASE + 0x710, &[0x11, 0x22, 0x33, 0x44]);
        image.write(BASE + 0x720, &[0x22, 0xaa]);
        for (index, source, destination, size, handler) in [
            ...
            image.write_u32(BASE + offset as u32, source);
            image.write_u32(BASE + offset as u32 + 4, destination);
            image.write_u32(BASE + offset as u32 + 8, size);
            image.write_u32(BASE + offset as u32 + 12, handler);
        }
        ...
        image.write(BASE + 0x800, &code_bytes);
        write_pal_table(
            &mut image,
            BASE + PAL_SLOT_TABLE_OFF,
            &[pal_slot("first_task"), pal_slot("second_task")],
    ...
    pub(crate) fn craft_ambiguous_pal_main_image() -> Vec<u8> {
        let mut image = PalImageBuilder::new();
        ...
        let (second, _) = assemble(
            BASE + 0x200,
            &pal_initializer_items(BASE + 0x8000, PAL_EXTENDED_CAPACITY),
        );
        image.write(BASE, &first);
        image.write(BASE + 0x200, &second);
        write_pal_table(&mut image, BASE + PAL_SLOT_TABLE_OFF, &[pal_slot("aa")]);
        write_pal_table(&mut image, BASE + 0x8000, &[pal_slot("bb")]);
        image.bytes
    }
}
