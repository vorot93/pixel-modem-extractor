//! Reconstruct a modem code image's firmware source tree from embedded __FILE__
//! strings. The image is usually MAIN, whose split name is model-dependent
//! (`02_MAIN` on mustang/S5400, `01_MAIN` on cheetah/S5300); the generated
//! labels name the actual image, derived from the input filename.
...
fn render_leaf(rec: &Record, image_name: &str) -> String {
    let mut lines = vec![
        format!("// Reconstructed node — NOT original source. From {image_name} __FILE__ strings."),
...
fn materialize_tree(
    ...
    for norm in norms {
        ...
        std::fs::write(&dest, render_leaf(rec, image_name))?;
    }
    if tree_dir.exists() {
...
fn write_summary_md(
    ...
    let mut lines = vec![
        format!("# {image_name} reconstructed source tree — summary"),
        String::new(),
    ];
    lines.push(format!(
    ...
    lines.extend([
        ...
        String::new(),
        format!(
            "- Reconstructed from `__FILE__` strings in {image_name}; **not original source**."
        ),
        "- Observable strings are heuristically proximity-attributed and UNVERIFIED.".to_string(),
        format!("- Scope: {image_name} only."),
    ]);
...
fn write_readme(out_root: &Path, image_name: &str, modem_label: Option<&str>) -> Result<()> {
    let descriptor = match modem_label {
        Some(g) => format!("Samsung Shannon {g} baseband image `{image_name}`"),
        None => format!("Samsung Shannon baseband image `{image_name}`"),
    };
    let text = format!(
        "\
# {image_name} reconstructed source tree
...
Re-run: `pixel-modem-extractor source-tree <{image_name}> --out <dir>`

Scope: {image_name} only.
"
    );
...
pub fn run(input: &Path, out: &Path, opts: &Opts) -> Result<PathBuf> {
    // The MAIN code image's split name is model-dependent (`02_MAIN` on
    // mustang/S5400, `01_MAIN` on cheetah/S5300). Derive it from the input
    // filename so the generated labels name the real image, not a hardcode.
    ...
    write_summary_md(out, &tree, &bare, &image_name)?;
    write_other_paths(out, &strings)?;
    write_readme(out, &image_name, opts.modem_label.as_deref())?;
...
mod tests {
    ...
    fn materialize_writes_tree() {
        ...
        let (written, modified) = materialize_tree(&recs, &tmp, "02_MAIN").unwrap();
        assert_eq!(written, 2);
    ...
    fn reports_write() {
        ...
        write_summary_md(&tmp, &_recs(), &HashMap::new(), "02_MAIN").unwrap();
        assert!(
        ...
        write_readme(&tmp, "02_MAIN", None).unwrap();
        assert!(
    ...
    fn readme_uses_modem_label_when_present() {
        ...
        write_readme(&tmp, "02_MAIN", Some("S5300")).unwrap();
    ...
    fn render_leaf_format() {
        ...
        assert!(!render_leaf(&rec2, "02_MAIN").contains("observable strings"));
    }
    ...
    fn materialize_rejects_escaping_norm() {
        ...
        let _ = std::fs::remove_dir_all(&tmp);
        let (written, _) = materialize_tree(&recs, &tmp, "02_MAIN").unwrap();
    ...
    #[test]
    fn labels_use_input_image_name_not_hardcoded_02_main() {
        // The source-tree labels (README, summary.md, leaf node comments) must
        // name the actual MAIN image derived from the input filename — never a
        // hardcoded "02_MAIN", which is false on models whose MAIN is 01_MAIN
        // (e.g. cheetah / S5300).
        let data = b"\x00../src/a.c\x00some message here\x00b/c.cpp\x00";
        let tmp = std::env::temp_dir().join("pme_st_imgname");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
        let input = tmp.join("01_MAIN");
        std::fs::write(&input, data).unwrap();
        let out = tmp.join("out");
        run(&input, &out, &Opts::default()).unwrap();

        for name in ["README.md", "summary.md", "tree/src/a.c"] {
            let content = std::fs::read_to_string(out.join(name)).unwrap();
            assert!(
                content.contains("01_MAIN"),
                "{name} should name 01_MAIN: {content}"
            );
            assert!(
                !content.contains("02_MAIN"),
                "{name} must not hardcode 02_MAIN: {content}"
            );
        }
    }
}
