...
- **global_shapes.json** — `pixel-modem-extractor-global-shapes-v1`; record-only
  ...
  byte offset, function contexts, and provenance paths. Conflicts group
  incompatible same-`(ISA, PC)` alternatives without choosing a winner.
  ...
  barriers separately. Quarantined producer records are never decoded; distinct
  ARM and Thumb identities may cover the same bytes and stay independent.
  `globals.json` is not rewritten. `--prune` keeps this sidecar.
