...
`thumb_functions.json`, final `symbols.json` (including authenticated exception/PAL evidence), and
the role manifests, but removes raw image slices plus `decompiled/thumb/` captures and carved
inputs. The retained final evidence remains usable after pruning, but standalone `symbolicate`
