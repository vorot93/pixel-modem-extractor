...
`01_MAIN`. `rf/` appears only when the model ships RF_CFG blobs (cheetah has none).
...
- **global_shapes.json** — `pixel-modem-extractor-global-shapes-v2`; record-only
  ...
  `--no-symbol-pass`, last — see Phase 3.2 above). One record per Recovered
