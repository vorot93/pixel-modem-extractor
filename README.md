...
`images/<MAIN>/pal_tasks/tasks.json`, a published startup-metadata artifact at
`images/<label>/startup_metadata/startup.json` for whichever image was discovered, and a published
debug-trace catalog at `images/<MAIN>/debug_traces/` on either model; all five terminal surfaces
survive `--prune`. `rf/`
