...
stronger: `__func__` > registration > exception root > PAL task > startup > token > string
reference; a stronger pass-2 primary does not remove the role label.
...
not parse it in shell; instead it requires process success, all three exports, and the exact five-line
`pixel-modem-extractor-ghidra-export-v5` marker that Java writes only after complete root postflight.
Generated pass-1 kits always write `startup_metadata=none` on that marker because
`ApplyStartupMetadata` is pass-2 only.
...
**Startup metadata (default on, after pass-1 inventories).** Phase 3B is not a generation-time
seeder. After a current pass-1 Ghidra inventory exists, `decompile --run` and `decompose` prove four
independent sections against that image's raw-plus-scatter runtime plus accepted Ghidra/Thumb
executions: hardware-init (`hw_Init`, only when uniquely reset-reachable), the stack-protection
...
`pixel-modem-extractor-startup-metadata-v1`; identity
`v1:<manifest-blake3>:<named-roots>:<no-return-roots>:<privileged-ops>`. Because the privileged-op
sweep always runs, a discovered image with all string sections absent is still `Present` with an
empty array. Opaque skips and generation-only `decompile` (no `--run`) do not discover.

On the normal `decompose` route, discovery runs after terminal raw/scatter/exception/PAL marshalling
and before symbol maps. Pass 2, still `-noanalysis`, inserts `ApplyStartupMetadata.java` after
`ApplySymbols.java`. It never creates functions, never re-analyzes, and never enables Repair Flow
Damage. It applies reserved-namespace role labels (`startup_hardware_init` /
`startup_stack_protection_failure`) and `Function.setNoReturn(true)` only when the artifact proved
non-return. `--no-symbol-pass` still writes the sidecar and attaches evidence, but does not spawn
the applicator or set Ghidra no-return.

Both retained MAIN images contain exactly one of each of the three production seeds. Private-corpus
goldens pin those seeds and leave section statuses, counts, identity, and manifest BLAKE3 as empty
sentinels until a lawful inventory-gated run prints `PIN OBSERVED` and those values are copied.
Unique seeds are not named roots: the 2026-08-27 probe did not close hardware-init reset
reachability or stack-handler non-return on either model. Never infer a corpus pass from an
unset-variable skip.
...
Both the normal and `--no-symbol-pass` finalizers consume the same immutable current role context;
neither reconstructs role authority from names, Ghidra ownership, pass-2 maps, or an older
`symbols.json`.
...
and a MAIN with a validated PAL task plan adds `pal_tasks/<label>/tasks.json` (see Formats). A
`--run` kit with current inventories may also add `startup_metadata/<label>/startup.json`. These
...
`decompose` writes (default `./<img>.decomposed/`; this shows the unpruned tree):
    ...
    │   │   ├── pal_messages/         # messages.json; retained by --prune (MAIN when recognized)
    │   │   ├── startup_metadata/     # startup.json after pass-1 inventories; retained by --prune
...
`images/<MAIN>/pal_tasks/tasks.json`, a published PAL-messages inventory at
`images/<MAIN>/pal_messages/messages.json` when the unique messaging initializer proves, a published
startup-metadata artifact at
`images/<label>/startup_metadata/startup.json` for whichever image was discovered, and a published
debug-trace catalog at `images/<MAIN>/debug_traces/` on either model; these terminal surfaces
survive `--prune`. `rf/`
appears only when the model ships RF_CFG blobs (cheetah has none). Pruning retains
`thumb_functions.json`, final `symbols.json` (including authenticated exception/PAL/startup
evidence), and the role manifests, but removes raw image slices plus `decompiled/thumb/` captures
and carved inputs. The retained final evidence remains usable after pruning, but standalone `symbolicate`
cannot be rerun because the raw authentication bytes are intentionally gone.
...
The `startup_metadata` stage follows inventories and precedes `symbol_map`. Completed discovery
emits `startup_hardware_init` / `startup_stack_guard` (canonical address or JSON `null`),
`startup_stack_non_return` (exclusive with a null stack guard), `startup_compiler_operands`, and
`startup_privileged_ops`; omitted means discovery did not complete, and `0` / `null` / `false` means
it completed with zero. Reason-only `startup_error` is exclusive with those counts. Pass-2
`startup_metadata_apply` (skipped for `--no-symbol-pass` or no Present artifact) reports
`startup_apply_labeled` / `startup_apply_no_return` with the same None-versus-Some(0) rule, and
reason-only `startup_apply_error`.
...
- **Architectural exception-root manifest** — `pixel-modem-extractor-exception-roots-v1`: the
  ...
  auto-analysis and records concrete function, primary-symbol, and role-label ownership; the v5
  ...
  marshalling completes, startup-metadata discovery publishes (or leaves Unmanaged), and immediately
  before symbol-map construction, `decompose` builds at most
  ...
  `blocks/*.bin` payload), and current exception/PAL/startup manifests into the existing Ghidra kit
  once, then derives both strict application contexts from that one runtime. Present startup
  authenticates terminal `startup.json` into kit `startup_metadata/<label>/startup.json`; not Present
  is `none` / `-`. It is not a snapshot of post-pass-2 output. Symbol maps bind these input
  identities. After invalidating stale exports, the host revalidates the complete snapshot and map
  binding immediately before spawning Ghidra; drift blocks the process and leaves no old export
  current. `report.json` carries one adjacent `exception_roots` stage plus an all-or-none per-image
  application-counter group; a reason-only `exception_error` is exclusive with those counters.
...
- **PAL messages manifest** — `pixel-modem-extractor-pal-messages-v1`: generation-time MAIN-only
  ...
  `PAL_MSG_MAX_ENTITY_COUNT` plus one initializer proof; slots are raw hashed descriptors. Phase 4
  first increment is parked (2026-09-05): both retained MAIN images prove clean absence (the unique
  hit is a log format, not a table initializer). Alternate unique hits (`pal_Init1`, the
  `PAL_QUEUE_FULL(%s, %s, %s)` log format) are not setup seeds. Entity, queue, and route graphs
  are not scheduled. No Ghidra apply script. Identity `v1:<manifest-blake3>:1:<slots>`.
...
- **Startup-metadata manifest** — `pixel-modem-extractor-startup-metadata-v1`: four independent
  ...
  binds the image/runtime/pass-1 inventory hashes, optional exception-root identity, tagged
  `hardware_init` / `stack_guard` / `compiler` sections (`absent` or `present`), a possibly empty
  `privileged_ops` array, and at most two Ghidra applications (`hw_Init`,
  `StackProtectionFailure`) with `set_no_return` only when local CFG proved non-return. RVCT
  operands stay uninterpreted (no major/minor/build). Identity is
  `v1:<manifest-blake3>:<named-roots>:<no-return-roots>:<privileged-ops>`. Pass 2 applies labels
  after `ApplySymbols` and never creates functions or re-enables Repair Flow Damage. The v5 export
  marker binds `startup_metadata=<identity-or-none>`; generated pass-1 kits write `none`.
